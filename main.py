...
if __name__ == '__main__':
    class LoadFromFile (argparse.Action):
        def __call__ (self, parser, namespace, values, option_string = None):
            with values as f:
                parser.parse_args(f.read().split(), namespace)  # parse arguments in the file and store them in the target namespace
    ...
    opt.seed = None
    opt.image = None  # image prompt
    opt.image_config = None  # image config csv
    opt.known_view_interval = 4  # train default view with RGB loss every & iters, only valid if --image is not None.
    opt.IF = False
    opt.dmtet = False
    opt.tet_grid_size = 128  # tet grid size
    opt.init_with = ''  # ckpt to init dmtet
    opt.lock_geo = False
    ...
    if opt.backbone == 'vanilla':  # 느린데 CUDA 확장 안해도 되는거
        from nerf.network import NeRFNetwork
    elif opt.backbone == 'grid':  # 빠른거
    ...
    if opt.six_views:
        guidance = None  # test에는 guidance model 로딩 불필요
        ...
        if opt.optim == 'adan':  # Adan usually requires a larger LR
