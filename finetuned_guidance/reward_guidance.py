...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    매개변수 (Args)
    - scheduler (`SchedulerMixin`): timesteps를 가져올 스케줄러.
    - num_inference_steps (`int`): Diffusion 단계 수 (`timesteps`와 동시 사용 불가).
    - device (`str` 또는 `torch.device`, 선택적): timesteps를 이동할 디바이스.
    - timesteps (`List[int]`, 선택적): 커스텀 timesteps 지정 (`num_inference_steps`와 동시 사용 불가).
